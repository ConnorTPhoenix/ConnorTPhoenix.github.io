...
## Hypothesis Testing
 I next wanted to test if any of these values were significantly different from the mean 'highest_rank' of 44.1. I selected genres with more than 9 songs, whose average 'highest_rank' was higher (lower numerically) than 44.1 ('Rock', 'Rock'n'Roll', 'Pop', and 'Latin').

We know that a frequency distribution of sample means from a sample distribution is normally distributed for a sufficiently large number of samples. There we can perform a one sample t-test to determine if an observed difference in our sample population (genre) is significantly different from our total population (all songs on the billboard 100).

For example, the 'Rock' genre consists of 103 songs. If we were to take an infinite number of samples of size 103 songs from the billboard 100 (2000) data set the frequency distribution of the average 'highest_rank' would be normally distribution. See below representation of 1,000,000 samples (N) of sample size 103 (n).

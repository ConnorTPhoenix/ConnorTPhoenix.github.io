...
## Hypothesis Testing
 I next wanted to test if any of these values were significantly different from the mean 'highest_rank' of 44.1 for the total data set. I selected genres with more than 9 songs, whose average 'highest_rank' was higher (lower numerically) than 44.1 ('Rock', 'Rock'n'Roll', 'Pop', and 'Latin').

Knowing the frequency distribution of sample means from a sample distribution is normally distributed for a sufficiently large number of samples, we can perform a one sample t-test.
...
#### HA: 'test genre' songs are not more likely to have a higher average 'highest_rank' to all songs that peaked on the billboard 100 in the year 2000.

Directionality was relevant because I only wanted to test if a genre is more likely to rank higher. Therefore I performed a one-tailed test t-test for each genre and yield the below results.
...
#### At an alpha level of .05 we rejected the null hypothesis for Rock'n'roll. Meaning that there is a difference between the average 'highest_rank' for Rock'n'roll compared to the total billboard hot 100 lis in 2000. In all other tests the we failed to reject the null hypothesis.
